...
resource "aws_vpc" "main" {
  ...
  tags = merge(var.tags, { Name = "${var.env}-codemind-vpc" })
}
...
resource "aws_subnet" "public" {
  ...
  tags = merge(var.tags, { Name = "${var.env}-public-${count.index}", Tier = "public" })
}
...
resource "aws_subnet" "private" {
  ...
  availability_zone = var.availability_zones[count.index]
  tags = merge(var.tags, { Name = "${var.env}-private-${count.index}", Tier = "private" })
}

resource "aws_eip" "nat" {
  count  = var.env == "prod" ? length(var.availability_zones) : 1
  domain = "vpc"
...
resource "aws_nat_gateway" "main" {
  ...
  tags          = merge(var.tags, { Name = "${var.env}-nat-${count.index}" })
}
...
resource "aws_security_group" "api" {
  ...
  description = "ECS API tasks — ALB inbound only"
  vpc_id      = aws_vpc.main.id
  ingress {
    from_port       = 3000
    to_port         = 3000
  ...
  tags = merge(var.tags, { Name = "${var.env}-api-sg" })
}

resource "aws_security_group" "database" {
  name        = "${var.env}-codemind-database"
...
resource "aws_security_group" "cache" {
  ...
  tags = merge(var.tags, { Name = "${var.env}-cache-sg" })
}
