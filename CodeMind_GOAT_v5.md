# StinKit — Final Build Specification
...
**StinKit is a local code graph with X-ray vision.**
...
| 2 | **Radical honesty** | Every output shows what StinKit knows AND what it doesn't. `Graph completeness: 83% · 417 ambiguous call sites`. This honesty IS the brand. |
| 3 | **Sub-second for daily use** | `stinkit check` (blast radius) returns in < 2 seconds. If it can't, it ships without that feature. Developers disable slow tools. |
...
| 5 | **One install, zero config** | `npx stinkit` indexes the repo and works. No config file. No API key. No account. Config exists for those who want it, never required. |
...
| 7 | **Honest about what tree-sitter misses** | Static analysis cannot see event emitters, DI containers, message queues, or dynamic dispatch. StinKit says so in every output. Teams can teach it via `connections.yaml`. |
...
```
stinkit             (no args → interactive: shows status + suggests next action)
stinkit index       Build or update the code graph
stinkit check       Blast radius of staged changes or a specific file
stinkit see         Compare an architecture diagram to actual code
stinkit trace       Trace an error backward to its probable cause
stinkit graph       Export or visualize the raw code graph
stinkit serve       Start MCP server for Claude Code integration
```
...
Walks the repo. Detects languages. Parses with tree-sitter. Builds a graph of every function, class, module, import, call site, and test file. Loads git history per node (who changed it, when, how often). Persists to `.stinkit/graph.msgpack` (MessagePack — 10x smaller and faster than JSON).
...
## Command: `stinkit check`

**The daily driver. Fast. Deterministic. Offline.**

```bash
stinkit check                      # Analyze staged changes
stinkit check --file src/auth.ts   # Analyze specific file
stinkit check --think              # Add Opus 4.7 failure analysis
stinkit check --report             # Generate static HTML report
```
...
The pre-commit hook (`stinkit check --install-hook`) ALWAYS runs Fast. If risk is HIGH or CRITICAL, it prints: `⚠ Run stinkit check --think for full analysis`. Never blocks for 30 seconds.
...
Teams can override thresholds in `.stinkit/config.yaml`:
```yaml
thresholds:
...
```
 ...
 ╰──────────────────────────────────────────────────────────╯
  ...
  ℹ Run stinkit check --think for failure scenario analysis
...
```bash
stinkit see diagram.png                     # Compare diagram to code
stinkit see diagram.png --scope services/   # Limit to a directory
stinkit see diagram.png --verify            # Show extraction before comparing
stinkit see diagram.png --output arch.mermaid   # Save corrected diagram
stinkit see diagram.png --ui                # Open side-by-side in browser
```
...
```
 ╭──────────────────────────────────────────────────────────╮
 │  STINKIT SEE — Extraction Preview                       │
 ╰──────────────────────────────────────────────────────────╯
...
// Persisted in .stinkit/see-mappings.yaml for future runs
...
```
 ╭──────────────────────────────────────────────────────────╮
 │  STINKIT SEE — Architecture Reality Check               │
 ╰──────────────────────────────────────────────────────────╯
  ...
  ℹ Corrected Mermaid diagram saved to .stinkit/architecture.mermaid
  ℹ Open with: stinkit see diagram.png --ui
```

---

## Command: `stinkit trace`

**The emergency tool. Post-incident root cause.**

```bash
stinkit trace "TypeError: Cannot read property 'token' of undefined"
stinkit trace --stack-trace error.log
stinkit trace "connection timeout" --days 30    # Wider lookback
```
...
**Never output confidence above 80%.** StinKit traces through code structure, not runtime execution. It doesn't know about deployment order, feature flags, infrastructure changes, or data state. Saying "94% confidence" from static analysis is dishonest.
...
## Command: `stinkit graph`

**The composability primitive. What makes this a platform, not just a tool.**

```bash
stinkit graph                          # Interactive terminal graph summary
stinkit graph --export mermaid         # Export as Mermaid diagram
stinkit graph --export dot             # Export as Graphviz DOT
stinkit graph --export json            # Export full graph as JSON
stinkit graph --export html            # Self-contained interactive HTML
stinkit graph --node src/auth.ts       # Show single node + its edges
stinkit graph --hotspots               # Top 20 highest-blast-radius nodes
```
...
```
 ╭──────────────────────────────────────────────────────────╮
 │  STINKIT HOTSPOTS — Highest Blast Radius Nodes          │
 ╰──────────────────────────────────────────────────────────╯
...
|---|---|---|
| `stinkit_check` | `stinkit check` | Blast radius analysis. Default: fast/offline. Add `think: true` for Opus enrichment. |
| `stinkit_see` | `stinkit see` | Diagram vs. code comparison. Requires image path. |
| `stinkit_trace` | `stinkit trace` | Error → root cause trace. |
| `stinkit_graph` | `stinkit graph` | Export or query the code graph. |
| `stinkit_status` | `stinkit` (no args) | Index health + completeness metric. |

### Skill File (`.claude/skills/stinkit.md`)

Teaches Claude Code WHEN to use each tool. Three key rules:

1. **Before calling any tool, call `stinkit_status`.** If the index is stale (>7 days), suggest re-indexing.
2. **Use `stinkit_check` whenever the developer modifies shared code** (anything with >5 callers). Skip for tests, docs, config.
3. **Use `stinkit_see` when an architecture diagram is referenced.** Use `stinkit_trace` when an error is pasted.
...
- MessagePack persistence (`.stinkit/graph.msgpack`)
- `stinkit index` command with progress bar
- `stinkit` (no args) shows status + completeness metric
...
- **Exit criterion:** `stinkit check` returns in < 2 seconds on demo repo. Output looks like the spec.
...
- MCP server (5 tools)
- Skill file
- `stinkit serve` command
- Auto-config in `stinkit index` ("Enable Claude Code integration? [Y/n]")
...
- Single-page React app for `stinkit see --ui` (side-by-side view)
- Static HTML report for `stinkit check --report`
- `stinkit graph --hotspots` (the engineering all-hands slide)
- `stinkit graph --export mermaid|dot|json|html`
...
- Submission package
- **Exit criterion:** Judge can `npx stinkit` on a fresh repo and see results in under 60 seconds.
...
### 0:00 — The Promise
"Every tool tells you what your code does. StinKit tells you what your code does that you DON'T KNOW."
...
Claude calls `stinkit_check` via MCP. Answers with the full analysis inline.
"No context switch. The code graph lives inside your AI editor."

### 2:50 — Close
"StinKit. A local code graph with X-ray vision. Offline-first. Radically honest. Built with Opus 4.7. Open source."
...
| Three-product branding | Modes are verbs (`check`, `see`, `trace`), not products. StinKit is one tool. |
...
| Graph completeness metric | The brand identity. Every output shows what StinKit knows and doesn't know. |
...
| `stinkit graph` command | The composability primitive. Export, query, visualize the raw graph. Makes StinKit a platform. |
| `--report` HTML output | Static HTML file. Opens in any browser. No React app needed for Impact/Trace. |
| Error handling contracts | `StinKitResult<T>` with success/partial/failed states. LLMs return garbage sometimes. |

---

## Prize Targeting

| Prize | What Wins It | StinKit Feature |
|---|---|---|
| **Most Creative Opus 4.7 Exploration** | "Made us feel something" | `stinkit see` — watching your architecture diagram get corrected in real-time. Nobody has done this. The 3.75MP vision upgrade makes it possible. |
...
| **Best Use of Claude Managed Agents** | "Long-running tasks you'd actually ship" | `stinkit check --think` as a CI step that runs on every PR and posts the analysis as a comment. Not a demo — something you'd actually wire into your pipeline. |
