# StinKit CLI — Comprehensive Test Results
**Date:** 2026-04-24  
...
### BUG-NEW-1: `stinkit serve` exits immediately [BLOCKER]
**Severity:** BLOCKER for MCP integration  
**Repro:** `cd ~/test-repos/got && stinkit serve`  
...
### BUG-NEW-2: `stinkit --json` flag on default status command does not produce JSON [MAJOR]
**Severity:** MAJOR  
**Repro:** `cd ~/test-repos/got && stinkit --json`  
...
### BUG-NEW-3: `stinkit check --file nonexistent.ts` exits 0 with LOW risk [MAJOR]
**Severity:** MAJOR  
**Repro:** `cd ~/test-repos/got && stinkit check --file does/not/exist.ts`  
...
### BUG-NEW-7: `.claude/skills/stinkit.md` not generated by indexer [MINOR]
**Severity:** MINOR  
**Repro:** Test 5.2 — `stinkit index` in any repo  
**Expected:** `.claude/skills/stinkit.md` created to enable MCP skill discovery  
**Actual:** Only `.stinkit/graph/` directory created  
...
`stinkit serve` exits immediately. If the demo involves showing Claude Code integration via MCP, this will fail live. The entire MCP angle of the pitch cannot be demonstrated. **Mitigation:** Fix before demo or pivot demo to CLI-only workflow.
...
DOT: starts with "digraph stinkit", 56 directed edges ✓
```
...
```
stinkit --help: OK ✓
stinkit check --help: OK ✓
stinkit see --help: OK ✓
stinkit trace --help: OK ✓
stinkit graph --help: OK ✓
stinkit index --help: OK ✓
stinkit serve --help: OK ✓
No "Unknown command", no TODO/placeholder text ✓
```

**Test 4.2 — --json flag consistency:**
```
stinkit --json → FAIL: outputs formatted text, not JSON ✗
stinkit check --json → valid JSON ✓
stinkit graph --hotspots --json → valid JSON ✓
```
...
cd /tmp && stinkit index → warns no .git, exits 0 (acceptable) ✓
check without index → "Run stinkit index first", exit 1 ✓
```
...
```
stinkit serve → exits immediately (code 0), no output ✗
stinkit serve --port 8765 → exits immediately (code 0) ✗
FAIL: MCP server does not stay running
```

**Test 5.2 — Skill file:**
```
ls ~/test-repos/got/.claude/skills/stinkit.md → No such file or directory ✗
