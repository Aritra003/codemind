          ...
          {reports.map(r => {
            ...
            return (
              <Link key={r.id} href={`/dashboard/reports/${r.id}`}
                className="bg-[var(--bg-glass)] backdrop-blur-xl border border-[var(--border-subtle)] rounded-[20px] p-5 flex items-center gap-4 hover:border-border-light hover:bg-surface-raised transition-all duration-150 group block">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="font-mono text-xs text-ink-muted mt-0.5">{new Date(r.createdAt).toLocaleString()}</p>
                </div>
                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                  <div className={`font-mono text-lg font-bold leading-none ${scoreClass}`}>{score}</div>
                  <div className="font-mono text-xs text-ink-muted">security</div>
                </div>
