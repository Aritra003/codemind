...
// Dual-canvas: glow canvas (CSS-blurred for bloom) + sharp canvas on top
...
export type RenderNode = d3.SimulationNodeDatum & {
  ...
  // Health fields — populated by graph API when a report exists
...
// Semantic severity — drives ring color when report data exists
...
// Ring color: severity when report exists; structural hotspot inDeg tiers otherwise
export function ringColor(n: RenderNode): string {
  if (n.severity) return SEVERITY_COLORS[n.severity]!;
  if (n.inDeg >= 20) return "#EF4444";
  if (n.inDeg >= 10) return "#F97316";
  if (n.inDeg >= 4)  return "#EAB308";
  return "#6366F1";
}

export function nodeR(inDeg: number): number { return 8 + Math.log2(1 + inDeg) * 5; }
...
export function drawEdges(
  ...
  for (const lk of links) {
    const s = lk.source as RenderNode, tgt = lk.target as RenderNode;
    const x1 = s.x ?? 0, y1 = s.y ?? 0, x2 = tgt.x ?? 0, y2 = tgt.y ?? 0;
    const [cx, cy] = bezierCP(x1, y1, x2, y2);
    const isCircular = lk.edgeType === "circular";
    const isActive   = sel !== null && (s.id === sel || tgt.id === sel);

    ctx.beginPath(); ctx.moveTo(x1, y1); ctx.quadraticCurveTo(cx, cy, x2, y2);

    if (isCircular) {
      // Animated dashed red — always visible regardless of selection
      const dashOffset = (t * 18) % 16;
      ctx.setLineDash([5, 3]); ctx.lineDashOffset = -dashOffset;
      ctx.strokeStyle = isActive ? rgba("#EF4444", 0.95) : rgba("#EF4444", 0.55);
      ctx.lineWidth = isActive ? 2 : 1.2;
      ctx.stroke();
      ctx.setLineDash([]); ctx.lineDashOffset = 0;
    } else if (isActive) {
      const gr = ctx.createLinearGradient(x1, y1, x2, y2);
      gr.addColorStop(0, rgba(nodeColor(s), 0.88));
      gr.addColorStop(1, rgba(nodeColor(tgt), 0.88));
      ctx.strokeStyle = gr; ctx.lineWidth = 1.8; ctx.stroke();
    } else if (sel !== null) {
      ctx.strokeStyle = "rgba(255,255,255,0.02)"; ctx.lineWidth = 0.4; ctx.stroke();
    } else {
      ctx.strokeStyle = "rgba(30,41,59,0.7)"; ctx.lineWidth = 0.75; ctx.stroke();
    }
  }
}
...
export function drawNodeGlow(
  ...
  for (const n of nodes) {
    ...
    const glow = ringColor(n);
    const isSel = n.id === sel;
    if (sel !== null && !isSel && !hl.has(n.id)) continue;
    const pulse = reducedMotion ? 0.7 : 0.5 + 0.5 * Math.sin(t * 1.1 + n.inDeg * 0.6);
    const gR    = r * (isSel ? 3.2 : 2.2);
    const alpha = isSel ? 0.8 : 0.22 + 0.14 * pulse;
    const gr    = ctx.createRadialGradient(x, y, 0, x, y, gR);
    gr.addColorStop(0, rgba(glow, alpha));
    gr.addColorStop(0.45, rgba(glow, alpha * 0.35));
    ...
    ctx.fillStyle = rgba(nodeColor(n), 0.55); ctx.fill();
  }
}

// ── Badge icons (drawn top-right of node) ─────────────────────────────────

function drawErrorBadge(ctx: CanvasRenderingContext2D, x: number, y: number, r: number, color: string) {
  const bx = x + r * 0.7, by = y - r * 0.7, br = 4.5;
  ctx.beginPath(); ctx.arc(bx, by, br, 0, Math.PI * 2);
  ctx.fillStyle = color; ctx.fill();
  ctx.strokeStyle = "#05050F"; ctx.lineWidth = 1.5; ctx.stroke();
  ctx.font = "bold 7px sans-serif";
  ctx.textAlign = "center"; ctx.textBaseline = "middle";
  ctx.fillStyle = "#FFFFFF"; ctx.fillText("!", bx, by + 0.5);
  ctx.textBaseline = "alphabetic";
}

function drawWarningBadge(ctx: CanvasRenderingContext2D, x: number, y: number, r: number) {
  const bx = x + r * 0.7, by = y - r * 0.7, bh = 9;
  ctx.beginPath();
  ctx.moveTo(bx, by - bh / 2);
  ctx.lineTo(bx + bh / 2, by + bh / 2);
  ctx.lineTo(bx - bh / 2, by + bh / 2);
  ctx.closePath();
  ctx.fillStyle = "#EAB308"; ctx.fill();
  ctx.strokeStyle = "#05050F"; ctx.lineWidth = 1; ctx.stroke();
  ctx.font = "bold 6px sans-serif";
  ctx.textAlign = "center"; ctx.textBaseline = "middle";
  ctx.fillStyle = "#000000"; ctx.fillText("!", bx, by + 1.5);
  ctx.textBaseline = "alphabetic";
}

function drawCircularBadge(ctx: CanvasRenderingContext2D, x: number, y: number, r: number) {
  const bx = x - r * 0.7, by = y - r * 0.7;
  ctx.font = "bold 9px sans-serif";
  ctx.textAlign = "center"; ctx.textBaseline = "middle";
  ctx.fillStyle = "#F97316"; ctx.fillText("↺", bx, by);
  ctx.textBaseline = "alphabetic";
}

// ── Sharp node layer ──────────────────────────────────────────────────────

export function drawNodeSharp(
  ctx: CanvasRenderingContext2D,
  nodes: RenderNode[],
  sel: string | null,
  hl: Set<string>,
  ...
  for (const n of nodes) {
    const x = n.x ?? 0, y = n.y ?? 0, r = nodeR(n.inDeg);
    const col   = nodeColor(n), ring = ringColor(n);
    ...
    if (!isGhost) {
      ...
      ctx.strokeStyle = isSel ? "rgba(255,255,255,0.95)" : rgba(ring, 0.4 + 0.5 * pulse);
      ctx.lineWidth   = isSel ? 2.5 : 1.5; ctx.stroke();
      ctx.beginPath(); ctx.arc(x - r * 0.27, y - r * 0.27, r * 0.22, 0, Math.PI * 2);
      ctx.fillStyle = "rgba(255,255,255,0.16)"; ctx.fill();
    }

    // Health badges — color alone is never the only signal (accessibility requirement)
    if (!isGhost) {
      if (n.severity === "CRITICAL" || n.severity === "HIGH") {
        drawErrorBadge(ctx, x, y, r, SEVERITY_COLORS[n.severity]!);
      } else if (n.severity === "MEDIUM") {
        drawWarningBadge(ctx, x, y, r);
      }
      if (n.hasCircularDep) drawCircularBadge(ctx, x, y, r);
    }

    // Keyboard focus ring — dashed white outline
    if (n.id === focusedId) {
    ...
    // Label — minimum 11px (design system baseline)
    if (r >= 9 && !isGhost) {
      const raw   = n.name ?? n.id.split("/").pop() ?? n.id;
      const label = raw.length > 16 ? raw.slice(0, 14) + "…" : raw;
      const fs    = Math.max(11, Math.min(13, r * 0.85));
      ctx.font = `${fs}px "JetBrains Mono", monospace`;
      ctx.textAlign = "center"; ctx.textBaseline = "top";
      ctx.fillStyle = isSel ? "rgba(255,255,255,0.95)" : "rgba(203,213,225,0.85)";
