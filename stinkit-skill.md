...
| `stinkit_impact` | Analyze blast radius of a code change | Before committing or pushing shared code |
| `stinkit_drift` | Compare an architecture diagram to actual code | When reviewing or planning architecture changes |
| `stinkit_forensics` | Trace a production error to its root-cause commit | When debugging errors that aren't immediately obvious |

## Decision Rules

### Always run `stinkit_status` first
Before calling any other StinKit tool, check that the index exists and is fresh. If status reports the index is older than 7 days, suggest the developer run `stinkit reindex` before proceeding.

### When to use `stinkit_impact`
USE when:
...
HOW to call:
```
stinkit_impact({ file_path: "src/auth/middleware.ts" })
```
...
HOW to call:
```
stinkit_drift({ image_path: "/path/to/architecture.png", scope: "src/services/" })
```
...
HOW to call:
```
stinkit_forensics({ error_message: "TypeError: Cannot read property 'token' of undefined", lookback_days: 14 })
```
...
- All StinKit tools query a LOCAL index on the developer's machine. No data is sent externally except to the Claude API for reasoning.
- The index must exist. If `stinkit_status` returns an error, guide the developer to run `stinkit setup` first.
- `stinkit_drift` requires an image file on disk. It cannot process URLs or images from the conversation context.
- `stinkit_forensics` works best with recent git history. Default lookback is 14 days. Increase with `lookback_days` if the incident may be older.
...
→ Call `stinkit_status` → if fresh, call `stinkit_impact` with the auth file → present risk analysis

**Pattern 2: Architecture review**  
Developer: "Here's our architecture diagram, is it still accurate?" (uploads image)
→ Call `stinkit_drift` with the image path → present accuracy score and divergences
...
→ Call `stinkit_forensics` with the error → present causal chain and prevention steps

**Pattern 4: Planning a refactor**
Developer: "I want to replace our session management. What's the impact?"
→ Call `stinkit_impact` on the session management files → if HIGH/CRITICAL, also call `stinkit_drift` to see if the architecture diagram reflects the actual session flow
