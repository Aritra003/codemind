...
function StepCard({ line }: { line: string }) {
  ...
  if (isRisk) return <p className={`font-mono text-xs ${riskColor} ml-4`}>{clean}</p>;
  if (clean.trim().startsWith("Rollback:")) return <p className="font-mono text-xs text-solar ml-4">{clean}</p>;
  if (clean.trim().startsWith("Files:") || clean.trim().startsWith("Effort:") || clean.trim().startsWith("Test:"))
    return <p className="font-mono text-xs text-ink-muted ml-4">{clean}</p>;
  if (clean.trim() === "") return <div className="h-1" />;
  return <p className="font-mono text-xs text-ink leading-relaxed ml-4">{clean}</p>;
}
    ...
    <div className="glass rounded-2xl border border-brand/20 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-surface-raised">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-xs text-ink-muted">{result.repoName}</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="font-mono text-[10px] text-ink-dim">{result.affected} files · {result.tiers} tiers</span>
          <span className="font-mono text-[10px] bg-brand/10 text-brand px-2 py-0.5 rounded border border-brand/20">{result.model}</span>
        </div>
      </div>
    ...
    <div className="p-6 lg:p-8 max-w-3xl">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="font-display text-xl font-bold text-ink">Refactor Plan</h1>
        </div>
      ...
      <div className="glass rounded-2xl p-6 mb-6 space-y-4">
        ...
        <div>
          <label className="font-mono text-xs text-ink-muted block mb-2">Refactor goal <span className="text-ink-dim ml-1">⌘↵ to submit</span></label>
        ...
        <button onClick={() => void generate()} disabled={loading || !goal.trim() || !hasApiKey}
          ...
          {loading ? <><Loader2 size={15} className="animate-spin" /> Planning with CodeMind…</> : <><Sparkles size={15} /> Generate Plan</>}
        </button>
      </div>
