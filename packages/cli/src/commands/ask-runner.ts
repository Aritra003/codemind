...
export async function runAsk(question: string, config: UserConfig): Promise<void> {
  if (!config.anthropic_api_key) {
    process.stderr.write(
      formatError('NO_API_KEY', '`ask` requires an Anthropic API key.', 'Set ANTHROPIC_API_KEY in ~/.codemind/config.yaml or as env var.') + '\n',
    )
    process.exit(1)
  }
  ...
  try {
    ...
    process.stdout.write(chalk.cyan(' │  CODEMIND ASK') + ' '.repeat(47) + chalk.cyan('│\n'))
